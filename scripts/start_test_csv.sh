...
kubectl cp "${jmx_dir}/${jmx_dir}.jmx" -n "jmeter" "$master_pod":/
...
for csvfilefull in "${jmx_dir}"/*.csv
  ...
  do
    ...
    kubectl -n "jmeter" cp "${jmx_dir}/${i}.csv" "${slave_pods[j]}":/
...
kubectl exec -ti -n "jmeter" "$master_pod" -- /jmeter/load_test "/${jmx_dir}.jmx"
